...
/**
  * This provides an event sourced behavior. It has a state, [[MyprojectState]], which
  ...
  */
object MyprojectBehavior {

  /**
    * Given a sharding [[EntityContext]] this function produces an Akka [[Behavior]] for the aggregate.
    */ 
  def create(entityContext: EntityContext[MyprojectCommand]): Behavior[MyprojectCommand] = {
    ...
    create(persistenceId)
      .withTagger(
        ...
        AkkaTaggerAdapter.fromLagom(entityContext, MyprojectEvent.Tag)
      )
  ...
  private[impl] def create(persistenceId: PersistenceId) = EventSourcedBehavior
      .withEnforcedReplies[MyprojectCommand, MyprojectEvent, MyprojectState](
        persistenceId = persistenceId,
        emptyState = MyprojectState.initial,
...
/**
  ...
  */
case class MyprojectState(message: String, timestamp: String) {
  def applyCommand(cmd: MyprojectCommand): ReplyEffect[MyprojectEvent, MyprojectState] =
    cmd match {
      case x: Hello              => onHello(x)
      case x: UseGreetingMessage => onGreetingMessageUpgrade(x)
    }

  def applyEvent(evt: MyprojectEvent): MyprojectState =
    evt match {
      case GreetingMessageChanged(msg) => updateMessage(msg)
    }
  private def onHello(cmd: Hello): ReplyEffect[MyprojectEvent, MyprojectState] =
    Effect.reply(cmd.replyTo)(Greeting(s"$message, ${cmd.name}!"))

  private def onGreetingMessageUpgrade(
    cmd: UseGreetingMessage
  ): ReplyEffect[MyprojectEvent, MyprojectState] =
    Effect
...
object MyprojectState {

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  def initial: MyprojectState = MyprojectState("Hello", LocalDateTime.now.toString)
  ...
  /**
    ...
    */
  val typeKey = EntityTypeKey[MyprojectCommand]("MyprojectAggregate")
  ...
  /**
    ...
    */
  implicit val format: Format[MyprojectState] = Json.format
}

/**
  * This interface defines all the events that the MyprojectAggregate supports.
  */
sealed trait MyprojectEvent extends AggregateEvent[MyprojectEvent] {
  def aggregateTag: AggregateEventTag[MyprojectEvent] = MyprojectEvent.Tag
}

object MyprojectEvent {
  val Tag: AggregateEventTag[MyprojectEvent] = AggregateEventTag[MyprojectEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends MyprojectEvent
...
/**
  ...
  */
trait MyprojectCommandSerializable

/**
  * This interface defines all the commands that the MyprojectAggregate supports.
  */
sealed trait MyprojectCommand
    extends MyprojectCommandSerializable
...
/**
  ...
  * application loader.
  */
object MyprojectSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    // state and events can use play-json, but commands should use jackson because of ActorRef[T] (see application.conf)
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[MyprojectState],
